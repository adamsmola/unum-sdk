...
static void dhcp_offer_rcv_cb(TPCAP_IF_t *tpif,
                        PKT_PROC_ENTRY_t *pe,
...
static PKT_PROC_ENTRY_t fp_dhcp_offer_pkt_proc = {
    0,
    {},
    0,
    {},
...
FP_TABLE_STATS_t *fp_dhcp_offer_tbl_stats(int reset)
{
    static FP_TABLE_STATS_t tbl_stats;
    memcpy(&tbl_stats, &dhcp_offer_tbl_stats, sizeof(tbl_stats));
    if(reset) {
        memset(&dhcp_offer_tbl_stats, 0, sizeof(dhcp_offer_tbl_stats));
    }
    return &tbl_stats;
}
...
FP_DHCP_OFFER_t **fp_get_dhcp_offer_tbl(void)
{
    return dhcp_offer_tbl;
}
...
void fp_reset_dhcp_offer_tables(void)
{
    int ii;
    ...
    for(ii = 0; ii < FP_MAX_DEV; ii++) {
        FP_DHCP_OFFER_t *item = dhcp_offer_tbl[ii];
        if(item) {
...
//  assigned to it)
static FP_DHCP_OFFER_t *add_dhcp(unsigned char *mac)
{
    int ii, idx;
    FP_DHCP_OFFER_t *ret = NULL;

    // Total # of add requests
    ++(dhcp_offer_tbl_stats.add_all);
    ...
    for(ii = 0; ii < (FP_MAX_DEV / FP_SEARCH_LIMITER); ii++) {
        ...
        if(!dhcp_offer_tbl[idx]) {
            ret = malloc(sizeof(FP_DHCP_OFFER_t));
            if(!ret) { // Run out of memory
                ++(dhcp_offer_tbl_stats.add_limit);
                return NULL;
            }
            ret->blob_len = 0;
            dhcp_offer_tbl[idx] = ret;
        }
        if(dhcp_offer_tbl[idx]->blob_len == 0) {
            ret = dhcp_offer_tbl[idx];
            memcpy(ret->mac, mac, 6);
            ++(dhcp_offer_tbl_stats.add_new);
            break;
        }
        // Check if this is the same MAC
        if(memcmp(dhcp_offer_tbl[idx]->mac, mac, 6) == 0) {
            ret = dhcp_offer_tbl[idx];
            ++(dhcp_offer_tbl_stats.add_found);
            break;
        }
    ...
    if(!ret) {
        ++(dhcp_offer_tbl_stats.add_busy);
    } else if(ii < 10) {
        ++(dhcp_offer_tbl_stats.add_10);
    }
                        ...
                        struct iphdr *iph)
{
    ...
    struct dhcp_offer_pkt *dhdr = ((void *)udph) + sizeof(struct udphdr);
    ...
    if(remains < sizeof(struct dhcp_offer_pkt) + 1) {
        DPRINTF("%s: incomplete DHCP header, %d bytes remains, need %d\n",
                   __func__, remains, sizeof(struct dhcp_offer_pkt) + 1);
        return;
    }
    // We only interested in DHCP offer packets
    if(dhdr->type != 2) {
        return;
    }
    // Verify DHCP cookie
    ...
    remains -= sizeof(struct dhcp_offer_pkt);
    ...
    if(size > FP_MAX_DHCP_OPTIONS) {
        DPRINTF("%s: too many DHCP options, %d of max %d, truncating\n",
                __func__, size, FP_MAX_DHCP_OFFER_OPTIONS);
        size = FP_MAX_DHCP_OPTIONS;
    }
    // Add/find the device and update its DHCP request info
    FP_DHCP_OFFER_t *fpd = add_dhcp(ehdr->h_source);
    if(!fpd) {
    ...
    fpd->blob_len = size;
    memcpy(fpd->bytes, dhdr, FP_DHCP_BYTES);
...
int fp_dhcp_offer_init(void)
{
    ...
    dhcp_offer_tbl = calloc(FP_MAX_DEV, sizeof(FP_DHCP_OFFER_t *));
    if(!dhcp_offer_tbl) {
